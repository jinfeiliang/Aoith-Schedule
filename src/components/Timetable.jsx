...
export default function App() {
  ...
  function getCurrentPeriod(currentHr, currentMin) {
    const HoursToMinute = (x) => {
      return x * 60;
    };
    let candidatePeriods = Period_Settings.filter(
      (index) =>
        HoursToMinute(index.from.hour) + index.from.minute <=
          HoursToMinute(currentHr) + currentMin &&
        HoursToMinute(index.to.hour) + index.to.minute >=
          HoursToMinute(currentHr) + currentMin,
    );
  ...
  function UpdateCurrentView() {
    ...
    Period_Settings.forEach((x) => {
      const Period = x.period;
      const Start_Time = (x.from.hour * 60) + x.from.minute;
      const Due_Time = (Start_Time) - (CurrentHour * 60 - CurrentMinute);
      console.log(Period + ":" + Due_Time);
      const Period_HTML = document.querySelector(`.Period${Period}`);
      const Period_Time_Due_HTML = Period_HTML.querySelector(".Periods-Due");
      if (Due_Time > 0) {
        Period_Time_Due_HTML.innerHTML = `In ${Due_Time} minutes`;
      } else if (Due_Time < 0) {
        Period_Time_Due_HTML.innerHTML = `${-Due_Time} minutes ago`;
      } else if (Due_Time == 0) {
        Period_Time_Due_HTML.innerHTML = `now`;
      }
    });
  }
  ...
  return (
    <>
      <button className="p-6 bg-green-100" onClick={() => UpdateCurrentView()}>
        Update
      </button>
      <table>
        <thead>
        ...
        <tbody>
          <tr className="Periods Period1">
            ...
            <td>9:25 AM</td>
            <td>45 minutes</td>
            <td className="Periods-Due">CALCULATING</td>
          </tr>
